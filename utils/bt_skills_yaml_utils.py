"""
Bluetooth Skills YAML lifecycle helpers.
...
``bt_skills_`` prefix so the two domains coexist in the same
...
The "active source" flag is independent of the Wi-Fi one — flipping the
BT bot between cloud/user does not affect the Wi-Fi bot.
"""
...
from configs.path_configs import (
    ...
    BT_SKILLS_YAML_DATED_RE,
    BT_SKILLS_YAML_DATED_TEMPLATE,
)
...
def get_active_source() -> str:
    """Return the current BT active source, one of {"cloud", "user"}."""
    return _active_source


def set_active_source(source: str) -> str:
    """Set the BT active source. Invalid values are normalised to "cloud"."""
...
def parse_dated_filename(filename: str) -> Optional[date]:
    """Return the ISO date encoded in `bt_skills_YYYY-MM-DD.yaml`, else None."""
...
def find_latest_dated_yaml(directory: str | Path) -> Tuple[Optional[Path], Optional[date]]:
    """
    Scan `directory` for files matching `bt_skills_YYYY-MM-DD.yaml` and
...
def today_dated_filename(d: Optional[date] = None) -> str:
    """Canonical BT filename for a YAML produced today (or `d` if provided)."""
    return BT_SKILLS_YAML_DATED_TEMPLATE.format(date=(d or date.today()).isoformat())
...
def find_latest_cloud_baseline_yaml() -> Tuple[Optional[Path], Optional[date]]:
    """Latest dated BT YAML in the local `cloud/` mirror."""
    return find_latest_dated_yaml(local_cloud_baseline_dir())


def find_latest_user_yaml() -> Tuple[Optional[Path], Optional[date]]:
    """Latest dated BT YAML in the local `user/` overrides dir."""
    return find_latest_dated_yaml(local_user_overrides_dir())


def find_latest_share_yaml() -> Tuple[Optional[Path], Optional[date]]:
    """
    Latest BT YAML on the SHARE FOLDER. Falls back to the legacy un-dated
    `bt_skills.yaml` when no dated file exists yet. Returns (None, None)
    when the share is unreachable or contains no BT YAML at all.
    """
    ...
    if path is not None:
        return (path, dt)
    legacy = Path(cloud_dir) / BT_SKILLS_YAML_FILENAME
    if legacy.exists():
        return (legacy, None)
    return (None, None)
...
def current_active_yaml() -> Tuple[Optional[Path], Optional[date], str]:
    """
    Return `(path, date, effective_source)` of the BT YAML that should be
    ...
    # Final fallback: legacy un-dated bt_skills.yaml directly under
    # the local skills_config root (mirrors WiFi's same-name fallback).
    legacy = local_cloud_baseline_dir().parent / BT_SKILLS_YAML_FILENAME
    if legacy.exists():
...
def refresh_local_cloud_baseline() -> Tuple[Optional[Path], Optional[date]]:
    """
    Pull the latest BT YAML from the share folder into the local `cloud/`
    dir, replacing any older mirror files with the bt_ prefix. Best-effort:
    returns (None, None) when off-VPN. The WiFi skills files in the same
    ...
    # Materialise the date — if share file was legacy un-dated bt_skills.yaml,
    ...
    # Skip the copy when the mirror already holds this exact file — see the
    # matching comment in skills_yaml_utils.refresh_local_cloud_baseline().
    ...
    if not already_current:
        try:
            ...
            print(f"[bt_skills_yaml] refresh_local_cloud_baseline failed: {e}")
            return (None, None)

    # Prune older BT mirror files so cloud/ holds a single BT baseline.
    # IMPORTANT: only prune files matching the bt_skills_ prefix — never
    # touch the WiFi skills_*.yaml mirror that lives in the same dir.
    try:
...
# --- Status summary (for future BT side-panel) --------------------------

def skills_yaml_status() -> dict:
    """
    Build the payload for a future BT chatbot status/init dialog. Mirrors
    `utils.skills_yaml_utils.skills_yaml_status` but for BT files.
    """
