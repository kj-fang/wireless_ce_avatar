...
def pick_local_analysis_file():
    ...
    try:
        ...
        selected_path = filedialog.askopenfilename(
            ...
            filetypes=[
                ('Supported files', '*.zip *.7z *.rar *.log *.hci.txt *.etl *.etl.* *.bin *.dmp'),
                ('All files', '*.*'),
            ],
        )
        root.destroy()
        ...
        if not _is_allowed_local_analysis_filename(selected_name):
            return jsonify({
                'success': False,
                'message': f'Invalid file type: {selected_name}. Only .zip, .7z, .rar, .etl, .bin, .hci.txt, .log, or .dmp are allowed.'
            }), 400
...
def upload_local_analysis():
    ...
    if not _is_allowed_local_analysis_filename(original_name):
        return jsonify({
            'success': False,
            'message': f'Invalid file type: {original_name}. Only .zip, .7z, .rar, .etl, .bin, .hci.txt, .log, or .dmp are allowed.'
        }), 400
    ...
    elif source_lower.endswith('.etl') or bool(re.search(r'\.etl\.\d+$', source_lower)) or source_lower.endswith('.bin'):  # Wi-Fi .etl or ddd
        resp['use_chatbot'] = True
...
def open_local_analysis():
    ...
    if not _is_allowed_local_analysis_filename(original_name):
        flash(f'Invalid file type: {original_name}. Only .zip, .7z, .rar, .etl, .bin, .hci.txt, .log, or .dmp are allowed.', 'danger')
        return redirect(url_for('main.index'))
