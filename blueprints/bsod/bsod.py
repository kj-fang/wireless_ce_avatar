...
def bsod_submit():
    ...
    try:
        ...
        if response.status_code == 200:
            flash("BSOD Analysis Form Submitted Successfully! The analysis results will be emailed to you soon.", "success")
            print("submit bsod success!")
            return redirect(url_for('main.download_result_bsod'))
        else:
            # Full detail goes to the server log (only devs see this)
            print(f"BSOD API failed: HTTP {response.status_code}, body={response.text}")
            # User sees a short, safe message
            flash(f"Failed to submit BSOD analysis (HTTP {response.status_code}). Please try again later.", "error")
            return redirect(url_for('main.download_result_bsod'))
