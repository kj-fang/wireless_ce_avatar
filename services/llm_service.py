...
class _AnthropicMessageAdapter:
    def __init__(self, response):
        ...
        self.content = "".join(text_blocks) if text_blocks else None
    ...
    def create(self, model, messages, temperature=1.0, top_p=1.0,
        ...
        if frequency_penalty is not None or presence_penalty is not None:
            ...
            logging.getLogger(__name__).warning(
                "frequency_penalty and presence_penalty are not supported by the Anthropic API "
                "and will be ignored (model=%s).", model
            )
