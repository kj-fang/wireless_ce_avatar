"""
Sleepstudy Skills YAML lifecycle helpers.

Parallel to `utils.skills_yaml_utils` (which handles Wi-Fi skills) — same
on-disk layout, same lookup semantics, but every filename uses the
``sleepstudy_skills_`` prefix so the two domains coexist in the same
``skills_config/cloud/`` and ``skills_config/user/`` sub-folders without
colliding.

Wi-Fi files:       ``skills_YYYY-MM-DD.yaml`` / legacy ``skills.yaml``
Sleepstudy files:  ``sleepstudy_skills_YYYY-MM-DD.yaml`` / legacy ``sleepstudy_skills.yaml``

The "active source" flag is independent of the Wi-Fi and BT ones — flipping the
Sleepstudy bot between cloud/user does not affect the Wi-Fi or BT bot.
"""
...
from configs.path_configs import (
    ...
    SLEEPSTUDY_SKILLS_YAML_DATED_RE,
    SLEEPSTUDY_SKILLS_YAML_DATED_TEMPLATE,
)
from utils import helpers
from utils.skills_yaml_utils import (
    # Re-use the same on-disk roots so cloud/ and user/ are shared with
    # the WiFi skill files. Only the filename prefix distinguishes domain.
    local_cloud_baseline_dir,
    local_user_overrides_dir,
)


_DATED_RE = re.compile(SLEEPSTUDY_SKILLS_YAML_DATED_RE)


# --- Active source flag --------------------------------------------------
#
# Process-wide; resets to "cloud" on every app restart. Independent of the
# WiFi and BT active-source flags.
...
def get_active_source() -> str:
    """Return the current Sleepstudy active source, one of {"cloud", "user"}."""
    return _active_source


def set_active_source(source: str) -> str:
    """Set the Sleepstudy active source. Invalid values are normalised to "cloud"."""
...
def parse_dated_filename(filename: str) -> Optional[date]:
    """Return the ISO date encoded in `sleepstudy_skills_YYYY-MM-DD.yaml`, else None."""
...
def find_latest_dated_yaml(directory: str | Path) -> Tuple[Optional[Path], Optional[date]]:
    """
    Scan `directory` for files matching `sleepstudy_skills_YYYY-MM-DD.yaml` and
...
def today_dated_filename(d: Optional[date] = None) -> str:
    """Canonical Sleepstudy filename for a YAML produced today (or `d` if provided)."""
    return SLEEPSTUDY_SKILLS_YAML_DATED_TEMPLATE.format(date=(d or date.today()).isoformat())


# --- Directory layout ----------------------------------------------------
# Cloud and user dirs are SHARED with the WiFi side — only filename
...
def find_latest_cloud_baseline_yaml() -> Tuple[Optional[Path], Optional[date]]:
    """Latest dated Sleepstudy YAML in the local `cloud/` mirror."""
    return find_latest_dated_yaml(local_cloud_baseline_dir())


def find_latest_user_yaml() -> Tuple[Optional[Path], Optional[date]]:
    """Latest dated Sleepstudy YAML in the local `user/` overrides dir."""
    return find_latest_dated_yaml(local_user_overrides_dir())


def find_latest_share_yaml() -> Tuple[Optional[Path], Optional[date]]:
    """
    Latest Sleepstudy YAML on the SHARE FOLDER. Falls back to the legacy un-dated
    `sleepstudy_skills.yaml` when no dated file exists yet. Returns (None, None)
    when the share is unreachable or contains no Sleepstudy YAML at all.
    """
    ...
    if path is not None:
        return (path, dt)
    legacy = Path(cloud_dir) / SLEEPSTUDY_SKILLS_YAML_FILENAME
    if legacy.exists():
        return (legacy, None)
    return (None, None)
...
def current_active_yaml() -> Tuple[Optional[Path], Optional[date], str]:
    """
    Return `(path, date, effective_source)` of the Sleepstudy YAML that should be
    ...
    # Final fallback: legacy un-dated sleepstudy_skills.yaml directly under
    # the local skills_config root (mirrors WiFi's same-name fallback).
    legacy = local_cloud_baseline_dir().parent / SLEEPSTUDY_SKILLS_YAML_FILENAME
    if legacy.exists():
...
def refresh_local_cloud_baseline() -> Tuple[Optional[Path], Optional[date]]:
    """
    Pull the latest Sleepstudy YAML from the share folder into the local `cloud/`
    dir, replacing any older mirror files with the sleepstudy_skills_ prefix. Best-effort:
    returns (None, None) when off-VPN. The WiFi and BT skills files in the same
    ...
    # Materialise the date — if share file was legacy un-dated sleepstudy_skills.yaml,
    # stamp it as today's date so the on-disk layout stays consistent.
    ...
    try:
        ...
        print(f"[sleepstudy_skills_yaml] refresh_local_cloud_baseline failed: {e}")
        return (None, None)

    # Prune older Sleepstudy mirror files so cloud/ holds a single Sleepstudy baseline.
    # IMPORTANT: only prune files matching the sleepstudy_skills_ prefix — never
    # touch the WiFi skills_*.yaml or BT bt_skills_*.yaml mirrors that live in the same dir.
    try:
        for entry in target_dir.iterdir():
            if entry.is_file() and entry.name != target.name \
                    and entry.name.startswith("sleepstudy_skills_") and entry.suffix == ".yaml":
                entry.unlink()
    except OSError:
        pass
...
# --- Status summary (for future Sleepstudy side-panel) --------------------------

def skills_yaml_status() -> dict:
    """
    Build the payload for a future Sleepstudy chatbot status/init dialog. Mirrors
    `utils.skills_yaml_utils.skills_yaml_status` but for Sleepstudy files.
    """
